...
from tensorflow.keras.layers import Dense, Conv1D,LSTM ,Flatten, MaxPool2D, Dropout

model = Sequential()
model.add(Conv1D(64, 2, input_shape=(3072,1)))
model.add(Flatten())
...
cp = ModelCheckpoint(monitor='val_loss', save_best_only=True, mode='auto', 
                        filepath='./_save/ModelCheckPoint/keras48_9_cifar100_MCP.hdf5')


import time
start_time = time.time()
hist = model.fit(x_train,y_train, epochs=100, batch_size=64, validation_split=0.25, callbacks=[es,cp,tensorboard_callback])
end_time = time.time() - start_time
model.save('./_save/ModelCheckPoint/keras48_9_cifar100_model_save.h5')

#4. 평가, 예측
loss = model.evaluate(x_test,y_test, batch_size=612) 
