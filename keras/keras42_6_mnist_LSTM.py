...
model = Sequential()
model.add(LSTM(units = 128, activation='relu', input_length=28, input_dim=28)) 
...
import time
st = time.time()
model.fit(x_train,y_train, epochs=1000, batch_size=500, validation_split=0.2)
ed = time.time() - st
print('걸린 시간' , ed)
